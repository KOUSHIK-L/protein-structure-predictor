...
def render_mol(pdb, color_mode="Spectrum"):
    ...
    pdbview.zoom(2, 800)
    pdbview.spin(True)
    showmol(pdbview, height=520, width=800)
...
with st.sidebar:
    ...
    # ── Predict ────────────────────────────────────────────────────────────────
    predict = st.button("🔬  Predict Structure", disabled=(txt is None), use_container_width=True)
    ...
    st.markdown("""
    <div style="text-align:center;color:#94a3b8;font-size:0.68rem;padding:1.2rem 0 0.5rem">
        Powered by <a href="https://esmatlas.com" target="_blank"
        style="color:#94a3b8">Meta ESMFold</a>
    </div>
    """, unsafe_allow_html=True)


# ═══════════════════════════════════════════════════════════════════════════════
# MAIN AREA — Top navigation
# ═══════════════════════════════════════════════════════════════════════════════
        ...
        with tab2:
            if not plddt_df.empty:
                with st.container(border=True):
                    ...
                    st.line_chart(plddt_df, use_container_width=True, height=300)
                    st.caption(
        ...
        with tab3:
            if ss_counts:
                with st.container(border=True):
                    ...
                    st.markdown("<br>", unsafe_allow_html=True)
                    st.bar_chart(pd.DataFrame({"Residues": ss_counts}), use_container_width=True, height=280)
            ...
            if folded_seq:
                with st.container(border=True):
                    ...
                    with chart_col:
                        ...
                        st.bar_chart(get_aa_composition(folded_seq), use_container_width=True, height=320)
                    with seq_col:
